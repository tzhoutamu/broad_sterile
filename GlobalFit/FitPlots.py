import sys
import os
homedir = os.path.realpath(__file__)[:-len('GlobalFit/FitPlots.py')]
...
dir = homedir + 'GlobalFit/PlotData/'
...
data_WP = txt_to_array(dir+'WPSterileChi2'+suffix+'.dat')
...
figBF.suptitle(r'Best fit:  $\Delta m^2_{41} = %.2f \textrm{ eV}^2$, $\sin^2 2\theta_{14} = %.3f$. Total $\chi^2 = %.2f$'%(bestfit[0],bestfit[1], bestfit[2]), fontsize = titlesize)
figBF.savefig(plotdir+'WPContour_bestfit'+suffix+'.png')
...
figNH.savefig(plotdir+'WPContour_nullhyp'+suffix+'.png')
...
ax_comp.legend(handles = legend_elements, loc = 'lower right', fontsize = 16)
