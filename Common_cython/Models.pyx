...
class WavePacketSM:
    ...
    def oscProbability(self,double E, double l):
        ...
        L = 5.06773e6*l # conversion from meters to 1/eV
        prob = 1.
        prob -= sin(2*self.th12)**2*cos(self.th13)**4*(1-self.fosc(enu,L,self.dm2_21))/2.
        prob -= sin(2*self.th13)**2*(cos(self.th12)**2*(1-self.fosc(enu,L,self.dm2_31))+
...
class PlaneWaveSterile_full:
    def __init__(self):
        ...
        self.theta34 = np.arcsin(sqrt(r34))/2.
        self.dm2_31 = 2.44e-3 # eV^2
        self.dm2_21 = 7.42e-5 # eV^2
    ...
    def get_mixing_matrix(self):
        ...
        s13 = sin(self.theta13)
        s23 = sin(self.theta23)
        c14 = cos(self.theta14)
        c24 = cos(self.theta24)
        c34 = cos(self.theta34)
        s14 = sin(self.theta14)
        s24 = sin(self.theta24)
        s34 = sin(self.theta34)
        dCP = self.deltaCP
...
class WavePacketSterile_full:
    def __init__(self):
        ...
        self.delta34 = 0.
        self.theta12 = np.arcsin(sqrt(0.304))
        self.theta13 = np.arcsin(sqrt(0.02221))
        self.theta23 = np.arcsin(sqrt(0.570))
        self.theta14 = np.arcsin(sqrt(r14))/2.
        self.theta24 = np.arcsin(sqrt(r24))/2.
        self.theta34 = np.arcsin(sqrt(r34))/2.
...
class BroadSterile:
    def __init__(self,Sin22Th14 = 0.01,DM2_41 = 0.1, bvalue = 1):
        self.th14 = np.arcsin(sqrt(Sin22Th14))/2.
        self.th13 = np.arcsin(sqrt(0.0868525))/2.
        self.th12 = 0.583763
        self.dm2_41 = DM2_41
...
#This class uses np.sinc to calculate probability when b=0.
class BroadSterileNull:
    def __init__(self,Sin22Th14 = 0.01,DM2_41 = 0.1, bvalue = 1):
        self.th14 = np.arcsin(sqrt(Sin22Th14))/2.
        self.th13 = np.arcsin(sqrt(0.0868525))/2.
        self.th12 = 0.583763
