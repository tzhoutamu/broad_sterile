...
homedir = os.path.realpath(__file__)[:-len('PROSPECT/FitPlotsGlobal2-10.py')]
...
plotdir = homedir+'PROSPECT/Figures/Global_2-10/'
...
def getChi2(mass,angl,b,broad_sterile = False):
    ...
    chi2 = fitter_pros.get_chi2(model)+fitter_daya.get_poisson_chi2(model)+fitter_neos.get_chi2(model)
    return chi2
...
chi2_PW = np.add(np.load(datadir_pros+'PWSterileChi2_noint.npy'),np.load(datadir_daya+'PWSterileChi2_noint.npy'),np.load(datadir_neos+'PWSterileMass_p2.npy'))
...
chi2_BS = np.add(np.load(datadir_pros+'BroadSterileChi2_2-10.npy'),np.load(datadir_daya+'BroadSterileChi2_2-10.npy'),np.load(datadir_neos+'BroadSterileChi2_2-10_noint.npy'))
...
ax_comp.annotate('Global', xy = (5e-3,1.5), size = 42)
